...
sig = [0 sigAnt sigmaQ zeros(1,length(LPost)) zeros(1,length(LDBR)), 0];
...
%}
% Load previous data
fileName = input('Enter filename for data storage: ', 's')
save(fileName)
...
if exist('storageAnt', 'var') == 0 || resetVar == 1
    storageAnt = LAnt;
end

if exist('storageActive', 'var') == 0 || resetVar == 1
    storageActive = LActive;
end


%GREATER OPTIMIZING LOOP
k = 0;
while k <= optNum
        k = k+1;


storageActive = currentOptimum(length(LAnt)+1:length(LAnt)+length(LActive));
storageAnt = currentOptimum(1:length(storageAnt));
%% Optimizing loop #1
k1 = 0;

% Optimize anterior region
anteriorOpt = @(x)TROptimSum(lbdMaxRef, [1 nAnt nQ], x, [0 sigAnt 0]);
variablesAnt = length(LAnt);
optionsAnt = gaoptimset('tolfun', 1e-10, 'populationsize', 100+length(storageAnt(:,1)), 'timelimit', inf, ...
                     'display', 'off', 'stalltimelimit', inf, 'useparallel', 'always', 'crossoverfraction', .9, ...
                     'initialPopulation', storageAnt);
sizeAnt = sum(LAnt);
currentSum = 0;
TOpt = 0;
RrevOpt = 0;
LB = ones(size(LAnt))*.2*mean(LAnt);
UB = ones(size(LAnt))*2*mean(LAnt);

while k1 <= optLayer1
    k1 = k1+1;
    optimalAnt = ga (anteriorOpt, variablesAnt, [], [], [], [], LB, UB, [], optionsAnt );
    
    [TInv RrevInv] = TROptim(lbdMaxRef, [1 nAnt nH], optimalAnt, [0 sigAnt 0]);
    T = 1/TInv;
    Rrev = 1/RrevInv;
    
    if (T*3.4+Rrev) > currentSum
        currentSum = T*3.4+Rrev;
        TOpt = T;
        RrevOpt = Rrev;
        currentAntOpt = optimalAnt;
        storageAnt = [storageAnt ; currentAntOpt];
    end
    
    if length(storageAnt(:,1)) >= 3;
        storageAnt = currentAntOpt;
    end
    
    display(' ')
    display(strcat('Iterations: ', num2str(k)))
    display(strcat('Current Transmission/ Reverse Reflection: ', num2str(T), ' / ', num2str(Rrev)))
    display(strcat('Optimal Transmission/ Reverse Reflection: ', num2str(TOpt), ' / ', num2str(RrevOpt)))  
    
end

% Store the optimized section for use in next loop
currentOptimum = [currentAntOpt, currentOptimum(length(currentAntOpt)+1:end)];
...
% Optimize anterior region
activeOpt = @(x)gainOptimSum(lbdMaxRef, [nL nActive nH], x, [0 sigmaQ 0]);
variablesActive = length(LActive);
optionsActive = gaoptimset('tolfun', 1e-10, 'populationsize', 100+length(storageActive(:,1)), 'timelimit', inf, ...
                     'display', 'off', 'stalltimelimit', inf, 'useparallel', 'always', 'crossoverfraction', .9, ...
                     'initialPopulation', storageActive);
sizeActive = sum(LActive);
gainActive = 0;
LB = ones(size(LActive))*.2*mean(LActive);
UB = ones(size(LActive))*2*mean(LActive);
% Sizes for quantum wells must remain static
for m = 1:length(LActive)
    if (sigmaQ(m) ~= 0)
        LB(m) = LActive(m);
        UB(m) = LActive(m);
    end
...
while k2 <= optLayer2
    k2 = k2+1;
    optimalActive = ga(activeOpt, variablesActive, [], [], [], [], LB, UB,[], optionsActive );
    newGainActive = 1/activeOpt(optimalActive);
    
    if newGainActive > gainActive
        gainActive = newGainActive;
        currentActive = optimalActive;
        storageActive = [storageActive ; currentActive];
    end
    ...
    display(' ')
    display(strcat('Iterations: ', num2str(k))) 
    display(strcat('Current gain: ', num2str(newGainActive)))
    display(strcat('Current Optimum: ', num2str(gainActive)))
    
end

% Store the optimized section for use in next loop
currentOptimum(length(LAnt)+1:length(LAnt)+length(LActive)) = currentActive;
...
variables = length(L);
options = gaoptimset('tolfun', 1e-16, 'populationsize', 20+length(storageBin(:,1)), 'timelimit', inf, ...
                     'display', 'iter', 'stalltimelimit', inf, 'useparallel', 'always', 'crossoverfraction', .7, ...
...
while k3 <= optLayer3
    tic
        k3 = k3+1;
        optimalLength = ga (gainFunNew, variables, [], [], [], [], LB, UB, [], options );
    ...
    if length(storage(:,1)) >= 3;
        storage = currentOptimum;
    end


    if k == optLayer3;
        storageBin = [storageBin; currentOptimum]
