...
lambdaWindowOpt = [ 974.9, 975.1]*1e-9;
lambdaStepsOpt = 50;
optNum = 5;                   % Number of loops that the optimizer conducts
optReset = 10;                   % Numer of loops before optimizer resets
...
variables = length(L);
options = gaoptimset('tolfun', 1e-2, 'populationsize', 5+length(storage(:,1)), 'timelimit', inf, ...
...
while k <= optNum
    ...
    display(strcat('New Optimum: ', num2str(newGain)))
