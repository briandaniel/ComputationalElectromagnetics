...
spaceSize = 2.2e-6;            % Space size (1 = 1meter)                  [USER DEFINED]
gridSize = 2000;               % Total number of cells                    [USER DEFINED]
dx = spaceSize/gridSize;       % Cell Size (1 = 1meter)                   
stopTime = 800;
% Define FDTD temporal space
maxIter = gridSize*200;          % Maximum number of iterations + 1         [USER DEFINED]
...
sourcePos = .3e-6 ;              % Location of source in meters             [USER DEFINED]
sourcePos = round(sourcePos/dx);
posMed = .5e-6;                  % Position of medium relative to source    [USER DEFINED]
                                % in meters
...
lH = lambdaMax/(4*nH);
lL = lambdaMax/(4*nL);
grateNum = 2;                   % Number of grating sets
...
% Error
% L = L+mean(L).*(rand(1,length(L))-.5).*.05;
...
width = 120;                    % Width of Gaussian                        [USER DEFINED]
maxT = width*5;
...
% First dielectric
sigma = sigH;                      % Conductivity of dielectric               [USER DEFINED]
...
% Second dielectric
sigma2 = sigL;                    % Conductivity of dielectric               [USER DEFINED]
...
for k = 1:length(positions);
    ...
    if mod(k,2) == 0;
    medVecAlpha(cPos:cPos + positions(k)-1) = alpha2*ones(1,positions(k));
    medVecBeta(cPos:cPos + positions(k)-1) = beta2*ones(1,positions(k));
...
%%%%%% MAIN SOLUTION LOOP %%%%%%
figure('outerposition', [710   500   800   600])
set(1,'color',[.9 .9 .9])
...
for k = 1:length(positions);
    ...
    if mod(k,2) == 0;
    rectangle('position',[cPos*disConv, -2, positions(k)*disConv, 10], 'facecolor', [.9 .9 .9])
    cPos = cPos + positions(k);
    else
    rectangle('position',[cPos*disConv, -2, positions(k)*disConv, 10], 'facecolor', [.7 .7 .7])
    cPos = cPos + positions(k);
    end
...
lH = grateSize1*dx;
N = (length(L)-1)/2;
n = [1, nH, repmat([nL,nH], 1, N), 1];        % indices for the layers
...
%% 
movie2avi(M, 'Gauss Pulse 1D Debye Media', 'fps', 20);
