...
spaceSize = 11.5e-6;            % Space size (1 = 1meter)                  [USER DEFINED]
...
posMed = 2e-6;                  % Position of medium relative to source    [USER DEFINED]
                                % in meters
...
positions = zeros(length(L),1);

for k = 1:length(L)   
    positions(k) = round(L(k)/dx);
...
f = 374e12;                     % Frequency                                 [USER DEFINED]
...
for k = 1:length(positions);
    ...
    if mod(k,2) == 0;
    rectangle('position',[cPos*disConv, -2, positions(k)*disConv, 10], 'facecolor', [0 .5 .7])
    cPos = cPos + positions(k);
    else
    rectangle('position',[cPos*disConv, -2, positions(k)*disConv, 10], 'facecolor', [0 0  .9])
    cPos = cPos + positions(k);
    end
...
% Wavelength domain    
    ...
    plot(lambdaDom*10^9, refl.^2, lambdaDom*10^9, trans.^2)
    xlabel('{\lambda} (nm)')
    ylabel('Transmitted/Reflected') 
...
lambdaWindow = [ 700, 1000]*1e-9;
lambdaSteps = 500;
correction = 0;

ppR = pchip(lambdaDom+ correction, refl.^2);
compare = ppval(ppR, linspace(lambdaWindow(1), lambdaWindow(2), lambdaSteps));
sig = zeros(1,length(n)+2)';

[lambdas, R, r, T, t] = transferMatrix(lambdaWindow(1), lambdaWindow(2), lambdaSteps, [1; n ;1], L, sig);
