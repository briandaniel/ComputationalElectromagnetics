...
% fq = cNot./lambdas;
% freqDomNew = linspace(min(fq),max(fq), 1e2);
% freqDomStep = freqDom(end) - freqDom(end-1);
% ppr = spline(fq, r);
% rOmega = ppval(ppr,freqDomNew);
% 
% 
% thetaR = phase(rOmega);
% thetaRdot = gradient(thetaR, freqDomStep);
% thetaRdotdot = gradient(thetaRdot,freqDomStep);
% figure
% plot(freqDomNew, thetaRdot)
...
% kappas = 2.*pi./lambdas;
% deltas = 2.*pi.*(nH*lH)./lambdas;
% phase = -tan((sqrt(kappas.^2-deltas.^2)./deltas).*coth(sqrt(kappas.^2-deltas.^2).*grateWidth));
