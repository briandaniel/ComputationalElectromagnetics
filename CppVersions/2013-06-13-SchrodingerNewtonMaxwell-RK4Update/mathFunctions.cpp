...
// "trapz": Estimates integral on evenly spaced grid by trapezoidal rule (1D)
...
// "trapz": Estimates integral on evenly spaced grid by trapezoidal rule (3D)
double	trapz3D	(double *F, int Nx, int Ny, int Nz, double dx, double dy, double dz)
{
	double *sumZ = new double [Ny*Nx];
	double *sumY = new double [Nx];
	for (int i = 0; i < (Nx); i++)
	{
		for (int j = 0; j < (Ny); j++)
		{
				...
				{
					int pos = k + j*Nz + i*Ny*Nz;
					sumZ[i*Ny + j] = sumZ[i*Ny + j] + (F[pos] + F[pos+1])/2;
				}
				sumZ[i*Ny + j] = sumZ[i*Ny + j]*dz;
		}
	}
	...
	for (int i = 0; i < (Nx); i++)
	{
		...
		{
				int pos = i*Ny + j;
				sumY[i] = sumY[i] + (sumZ[pos] + sumZ[pos+1])/2;
		}
		sumY[i] = sumY[i]* dy;
	}


	double summed = trapz(sumY, Nx, dx);
	return summed;
