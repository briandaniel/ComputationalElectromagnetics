...
// "trapz": Estimates volume integral using trapezoidal rule
double trapz3D(double *F, int Nx, int Ny, int Nz, double dx, double dy, double dz)
{
	double* Fdz = new double [Nx*Ny];
	double* Fdzdy = new double [Nx];
	...
	for (int i = 0; i < Nx; i++)
	{
		for (int j = 0; j < Ny; j++)
		{
			...
			{
				sum = sum + (F[k + j*Nz + i*Ny*Nz ] + F[k + j*Nz + i*Ny*Nz +1])/2;
			}
			Fdz[ j + i*Ny ] = sum*dz;
		}
	}
	...
	for (int i = 0; i < Nx; i++)
	{
		...
		{
			sum = sum + (Fdz [j + i*Ny ] + Fdz[ j+ i*Ny + 1])/2;
		}
		Fdzdy[i] = sum*dy;
	}
	...
	{
		sum = sum + (Fdzdy[i] + Fdzdy[i+1])/2.0;
	}

	double summed = dx*sum;
