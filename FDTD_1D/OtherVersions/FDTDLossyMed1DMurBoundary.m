...
%% Parameters
% Define FDTD Space
dx = .005   ;                    % Cell Size (1 = 1meter)                   [USER DEFINED]
dt = .5*dx/(cNot);              % Time step (1 = 1sec)                     [USER DEFINED]
Sc = cNot*dt/dx;                % Courant Number (only works in Vacuum)
gridSize = 1600;                 % Total number of cells                    [USER DEFINED]
maxIter = 2500;                 % Maximum number of iterations + 1         [USER DEFINED]
...
maxT = 300;                     % Decay of Gaussian                        [USER DEFINED]
...
posMed = 600;                   % Position of mediunm                      [USER DEFINED]
...
plot(detInitial, 0, 'ro', detPost, 0, 'g*' , detRef, 0, 'b+')
...
if n>maxIter-2;
    ...
    plot([.2 10], [2/3 2/3], 'k-.', [.2 10], [1/3 1/3], 'k-.', [.2 10], [1 1], 'k-.')
    ylabel('Transmitted/Reflected') 
    xlabel('Frequency (GHz)')
    legend('Transmittance', 'Reflectance', 'Sum')
    title('Transmission/Reflection')
    
peakFreq = num2str(round(primaryFreq/10^8)*10^-1);
ppLambda = num2str(round(pointsPerWave));
info1 = strcat('Points per wavelength = { } ', ppLambda, '{ } at peak frequency:{ }', peakFreq, 'GHz');
